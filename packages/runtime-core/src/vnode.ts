...
// 由于v-if和v-for是动态改变节点结构的两种可能方式，
// 因此一旦我们将v-if分支和每个v-for片段视为一个块，
// 就可以将模板划分为嵌套块，并在每个块内结点 结构将是稳定的。 
// 这使我们可以跳过大多数children的diff，而只担心动态节点（由补丁标志指示）。

// 缓存可被diff的block栈
const blockStack: (VNode[] | null)[] = []
// 当前正在被diff的block，即blockStack的栈顶元素
